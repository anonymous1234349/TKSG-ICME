...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        with open('/results/IU-xray/topic_data.json', 'w') as json_file:
        ...
        if n_gpu_use > n_gpu:
            self.logger.warning(
                "Warning: The number of GPU\'s configured to use is {}, but only {} are available " "on this machine.".format(
                    n_gpu_use, n_gpu))
            n_gpu_use = n_gpu
        device = torch.device(self.device if n_gpu_use > 0 else 'cpu')  # gpu
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, retrieval_image_feat, retrieval_report_feat, mask, concept_targets, topic_targets) in enumerate(self.train_dataloader):
            images, reports_ids, reports_masks,  = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device),
            if self.retrieval:  # retrieval?
                retrieval_image_feat = retrieval_image_feat.to(self.device)
                retrieval_report_feat = retrieval_report_feat.to(self.device)
                concept_targets = concept_targets.to(self.device)
            if self.segment:  # segment?
                mask = mask.to(self.device)
            if self.topic:
                topic_targets = topic_targets.to(self.device)
            output, probabilities, topic_probabilities = self.model(images, retrieval_image_feat, retrieval_report_feat, mask, reports_ids, mode='train')
            ...
            if self.retrieval:
                ...
                local_index_list = list(local_index.detach().cpu().numpy())
                local_index_list = [array.tolist() for array in local_index_list]
            if self.topic:
                topic_probabilities_list = list(topic_probabilities.detach().cpu().numpy())
                topic_probabilities_list = [array.tolist() for array in topic_probabilities_list]
            for id in enumerate(images_id_list):
                keyword = {}
                if self.retrieval:
                    keyword['keyword'] = value_list[id[0]]
                    keyword['local_index'] = local_index_list[id[0]]
                if self.topic:
                    keyword['topic'] = topic_probabilities_list[id[0]]
                guide[id[1]] = keyword

            loss = self.criterion(images_id, output, reports_ids, reports_masks, is_retrieval=self.retrieval, concept_targets=concept_targets, predict_concept=probabilities,\
                                                                                is_topic=self.topic, topic_targets=topic_targets, topic=topic_probabilities)
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, retrieval_image_feat, retrieval_report_feat, mask, concept_targets, topic_targets) in enumerate(self.val_dataloader):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device)
                if self.retrieval:
                    retrieval_image_feat = retrieval_image_feat.to(self.device)
                    retrieval_report_feat = retrieval_report_feat.to(self.device)
                ...
                output, output_probs, probabilities, topic_probabilities = self.model(images, retrieval_image_feat, retrieval_report_feat, mask, mode='sample')
