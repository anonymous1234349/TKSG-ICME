...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        self.tokenizer = tokenizer # ['clip_feat', 'finetune_clip', 'swin_transformer_base', 'swin_transformer_tiny', 'swin_transformer_small', 'ViT']
        self.vocab_size = len(tokenizer.idx2token)  # 4335
        self.input_encoding_size = args.d_model # 512
        self.rnn_size = args.d_ff # 512
        self.num_layers = args.num_layers # 3
        self.drop_prob_lm = args.drop_prob_lm # 0.5
        self.max_seq_length = args.max_seq_length # 100

        # if self.model_frame == 'finetune_clip':
        #     self.att_feat_size = args.d_vf
        # if self.model_frame == 'clip_feat':
        #     self.att_feat_size = 512
        # if self.model_frame == 'swin_transformer_base':
        #     self.att_feat_size = 1024
        # if self.model_frame in ['swin_transformer_small', 'swin_transformer_tiny']:
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1, global_guide=None, local_guide=None, topic=None):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, global_guide=global_guide, local_guide=local_guide, topic=topic)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}, global_guide=None, local_guide=None, topic=None):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, topic_new, topic_probabilities_new  = self._prepare_feature(fc_feats, att_feats, att_masks)
        if self.new_topic:
            topic = topic_new
        ...
        logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, global_guide=global_guide, local_guide=local_guide, topic=topic)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, global_guide, local_guide, topic = utils.repeat_tensors(beam_size,
                                                                                            [p_fc_feats, p_att_feats,
                                                                                                pp_att_feats, p_att_masks,
                                                                                                global_guide, local_guide, topic]
                                                                                                )
        self.done_beams = self.beam_search(state, logprobs, global_guide, local_guide, p_fc_feats, topic, p_att_feats, pp_att_feats, p_att_masks, opt=opt)
        ...
        return seq, seqLogprobs, topic_probabilities_new

    def _sample(self, fc_feats, att_feats, att_masks=None, update_opts={}, global_guide=None, local_guide=None, topic=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, att_masks, opt, global_guide=global_guide, local_guide=local_guide,topic=topic) # 改，114
        if group_size > 1:
