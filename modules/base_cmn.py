...
class Transformer(nn.Module):
    def __init__(self, args, encoder, decoder, src_embed, tgt_embed, cmn):
        ...
        if self.args.retrieval:
            self.layer_norm_retrieval = nn.LayerNorm(normalized_shape=512)
        if self.args.topic:
            self.layer_norm_topic = nn.LayerNorm(normalized_shape=512)
    def forward(self, src, tgt, src_mask, tgt_mask, memory_matrix, global_guide=None, local_guide=None,topic=None):
        return self.decode(self.encode(src, src_mask), src_mask, tgt, tgt_mask, memory_matrix=memory_matrix,\
                           global_guide=global_guide, local_guide=local_guide,topic=topic) # jump 72

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None, global_guide=None, local_guide=None, topic=None):
        ...
        # Memory querying and responding for textual features
        if self.args.global_guide and global_guide is not None:  # global_guide?
            embeddings = embeddings + global_guide.unsqueeze(1).expand_as(embeddings)
            # embeddings = self.layer_norm_retrieval(embeddings)

        if self.args.local_guide and local_guide is not None:  # local_guide?
            local_guide = self.tgt_embed(local_guide)

        if self.args.topic:
            embeddings = embeddings + topic.unsqueeze(1).expand_as(embeddings)
            embeddings = self.layer_norm_topic(embeddings)
        decoder_output = self.decoder(embeddings, memory, src_mask, tgt_mask, past=past, local_guide=local_guide)
...
class Decoder(nn.Module):
    def __init__(self, args, layer, N):
        ...
        self.local_guide = args.local_guide
        self.layers = clones(layer, N)
        self.norm = LayerNorm(layer.size)

    def forward(self, x, memory, src_mask, tgt_mask, past=None, local_guide=None):
        if self.local_guide and local_guide is not None:
            memory = torch.cat((memory, local_guide), dim=1)
            # memory = self.norm(memory)
            padding_tensor = torch.ones(src_mask.size(0), 1, local_guide.size(1)).to(src_mask.device)
            src_mask = torch.cat((src_mask, padding_tensor), dim=2)
...
class MultiThreadMemory(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, topk=32):
        ...
        self.h = h  # 8
        self.linears = clones(nn.Linear(d_model, d_model), 4)  # 512, 512
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)  # 0.1
        self.topk = topk #
...
class BaseCMN(AttModel):
    ...
    def make_model(self, args, tgt_vocab, cmn):
        c = copy.deepcopy
        ...
        model = Transformer(
            ...
            Decoder(args, DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout), self.num_layers),
    ...
    def __init__(self, args, tokenizer):
        ...
        self.topk = args.topk # 32

        tgt_vocab = self.vocab_size + 1 # 4336

        self.cmn = MultiThreadMemory(args.num_heads, args.d_model, topk=args.topk)

        self.model = self.make_model(args, tgt_vocab, self.cmn)
        self.logit = nn.Linear(args.d_model, tgt_vocab) # 512, 4336

        self.memory_matrix = nn.Parameter(torch.FloatTensor(args.cmm_size, args.cmm_dim)) # 2048, 512
        ...
        if self.new_topic:
            if self.dataset_name == 'mimic_cxr':
                ...
                self.topic_predict_0 = nn.Linear(512, 14)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        att_feats, seq, att_masks, seq_mask, topic_new, topic_probabilities_new = self._prepare_feature_forward(att_feats, att_masks)
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None): # sample 和 train 都要经过这一步
        ...
        if self.new_topic:  # fc_feats = 2048
            fc_feats = att_feats.mean(dim=1)
            topic_output = self.topic_predict_0(fc_feats)  # predict topic ,1024

            topic_probabilities_new = torch.sigmoid(topic_output)

            # print(topic_probabilities)
            topic_new = self.topic_linear(topic_probabilities_new)
        ...
        return att_feats, seq, att_masks, seq_mask, topic_new, topic_probabilities_new

    def _forward(self, fc_feats, att_feats, seq, att_masks=None, global_guide=None, local_guide=None, topic=None):
        att_feats, seq, att_masks, seq_mask, topic_new, topic_probabilities_new = self._prepare_feature_forward(att_feats, att_masks, seq)
        if self.new_topic:
            topic = topic_new
        out = self.model(att_feats, seq, att_masks, seq_mask, memory_matrix=self.memory_matrix, global_guide=global_guide, local_guide=local_guide, topic=topic) # jump 66
        outputs = F.log_softmax(self.logit(out), dim=-1)

        return outputs, topic_probabilities_new
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask, global_guide=None, local_guide=None, topic=None):
        if len(state) == 0:
            ys = it.unsqueeze(1)
        ...
        out, past = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device), past=past,
                                      memory_matrix=self.memory_matrix, global_guide=global_guide, local_guide=local_guide, topic=topic)
